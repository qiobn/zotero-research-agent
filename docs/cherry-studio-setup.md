...
| 你说... | AI 调用的工具 | 说明 |
|---|---|---|
| 帮我找关于城市公共服务的论文 | `search_papers` | 关键词+语义混合搜索，RRF 融合排序 |
...
| 这篇论文哪里讨论了 gravity model | `get_paper_content` | 语义搜索模式，返回最相关的段落+页码 |
| 看一下这篇论文第 4 页 | `get_paper_content` | 按页查看模式 |
| 我在这篇论文上划了哪些重点 | `get_paper_content` | include_annotations=true |
| 我在所有论文里标注过哪些关于 methodology 的内容 | `search_annotations` | 跨论文搜索高亮和批注 |

### 写作引用（WRITE）

| 你说... | AI 调用的工具 | 说明 |
|---|---|---|
| 我这段话能引哪些文献（附你写的段落）| `suggest_citations` | 分析草稿内容，推荐库中可引用论文+证据页码 |
| 把这 3 篇导出成 BibTeX | `export_bibliography` | 支持 BibTeX 和 plain 格式 |
| 帮我把这篇 arXiv 论文加到库里 | `add_paper` | 支持 DOI/arXiv ID/URL，自动拉元数据+下载 PDF |
...
3. **添加新论文**：可以直接说「帮我把 10.xxxx/yyyy 这篇加到库里」，AI 会自动拉元数据并尝试下载 PDF
4. **索引同步**：在 Zotero 里手动加了论文后，跟 AI 说「更新一下索引」→ 增量同步，只处理变化的论文
5. **写操作是安全的**：加笔记、改标签、管理集合、添加论文都默认只做预览，AI 会问你确认后才执行
6. **中英文混搜**：可以用中文问题搜索英文论文，反之亦然（bge-m3 原生支持 100+ 语言）
...
A: Zotero 本地 API 是只读的。要使用写操作（加笔记、改标签、添加论文、管理集合），需要在 `.env` 里配置 `ZOTERO_API_KEY` 和 `ZOTERO_LIBRARY_ID`。配置后系统自动启用 Hybrid 模式：读操作走本地 API（快），写操作走 Web API。预览功能无论是否配置 API key 都可用。
